...
def extract_features_by_linear_search_distance_maximization(img_pixels, class_label):
    ...
    while row != 100:
        ...
        x1, y1, x2, y2, got_two_points, best_dist = do_linear_search(img_pixels, x_low, x_high, y_low, y_high)
...
def sample_50_data_each_class_bismillah():
    ...
    for irow in range(df_samples.shape[0]):
        img_pixels = np.array(df_samples.iloc[irow, :], dtype=np.uint8).reshape((28, 28)) #28x28
        img_pixels = img_pixels[4:24, 4:24] # 20x20
        img_pixels = ndimage.zoom(img_pixels, 5, order=1) # bilinear interpolation 100x100
        ...
        sample = extract_features_by_random_2points_from_four_grids(img_pixels, class_label=train_labels.iloc[irow])
        # print(sample)
        samples.append(sample)
    print("saving samples ... ...")
    samples = np.array(samples)
    np.savetxt("../saved_data/random_2points_from_four_grids.csv", samples, fmt='%i', delimiter=",")
...
def sample_hand_crafted_data():
    ...
    for cls_lbl in range(10):
        df_samples = df_samples.append(df_train.loc[df_train['label'] == cls_lbl].sample(n=1), ignore_index = True)
    ...
    samples = []
    for irow in range(df_samples.shape[0]):
        pixels = np.array(df_samples.iloc[irow, :], dtype=np.uint8).reshape((28, 28)) #28x28
        pixels = pixels[4:24, 4:24] # 20x20
        # plt.imshow(pixels, cmap='gray')
        pixels = ndimage.zoom(pixels, 5, order=1) # bilinear interpolation 100x100
        window_name = 'img'
        ...
        # plt.show()
    samples = np.array(samples)
    np.savetxt("../saved_data/50_samples_each_class_hand_made_features.csv", samples, fmt='%i', delimiter=",")
...
def sample_50_data_each_class():
    ...
    samples = np.array(samples)
    np.savetxt("../saved_data/50_samples_each_class_random_selection_without_grid_division.csv", samples.astype(int), fmt='%i', delimiter=",")
...
def get_n_features(pixels, class_label, n=8):
    ...
    while(i!=n):
        ...
        point = []
        while True:
            x, y = np.random.randint(low=0, high=100), np.random.randint(low=0, high=100)
            if pixels[x, y]:
                point.append(x)
                point.append(y)
...
def run(data, labels):
    ...
    util = Util()
    model = util.load_model("../saved_models/entropy_model.sav")
...
def main():
    ...
    data, labels = get_data('../saved_data/random_2points_from_four_grids.csv')
    run(data, labels)
main()
